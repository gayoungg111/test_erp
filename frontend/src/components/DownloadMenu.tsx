...
export interface DownloadUserInfo {
  email: string;
  nickname: string;
}

interface DownloadMenuProps {
  onDownload: (format: DownloadFormat, user: DownloadUserInfo) => Promise<void>;
  disabled?: boolean;
...
export default function DownloadMenu({
  ...
  const handleModalSuccess = async (user: DownloadUserInfo) => {
    if (!pendingFormat) return;
    await onDownload(pendingFormat, user);
  };
